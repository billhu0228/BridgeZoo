"""由施工计划派生的"一次成桥"(完成态)模型组装。

:class:`bridgezoo.fem.staged.plan.StagedPlan` 在 :func:`build_staged_cantilever`
末尾会附带一个 :class:`OneShotState`(成桥完成态:所有梁段/拉索一次激活、用真实
支座替代合龙平衡力)。本模块把该状态翻译成与求解器无关的 :class:`StructuralModel`,
即可分别交给自研直接刚度法 :class:`~bridgezoo.fem.linear_frame.DirectStiffnessSolver`
或 :class:`~bridgezoo.fem.opensees_backend.OpenSeesSolver` 求解并交叉校核。
...
def build_oneshot_model(plan: StagedPlan, name: str | None = None) -> tuple[StructuralModel, dict]:
    """由施工计划的成桥状态构建完成态 :class:`StructuralModel`。

    Parameters
    ----------
    plan : StagedPlan
        必须带有 ``plan.oneshot``(由 :func:`build_staged_cantilever` 生成)。
    name : str, optional
        结构模型名;缺省由 plan 名派生。
    ...
    ------
    ValueError
        当 ``plan.oneshot`` 为 ``None``(该计划未定义成桥状态)。
    """
    if plan.oneshot is None:
        raise ValueError("staged plan does not define a one-shot state")
    oneshot = plan.oneshot

    model = StructuralModel(name=name or f"{oneshot.label}_{plan.name}")
    coords: dict[int, tuple[float, float]] = {}
    deck_ids: list[int] = []
    anchor_ids: list[int] = []
    cable_nodes: dict[int, tuple[int, int]] = {}

    for nd in oneshot.nodes:
    ...
    for node, ux, uy, rz in oneshot.supports:
        model.add_support(node, ux=ux, uy=uy, rz=rz)

    for fr in oneshot.frames:
