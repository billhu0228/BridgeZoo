...
每个循环严格分成两个独立子轮。第一轮固定根数，只调整 A/B 索力；A 追踪激活切线零位移，
...
应力指标严格改善就接受，允许位移暂时扰动；下一循环索力轮会在新根数和新刚度下重新调平。
因此通常全新首循环需要3个完整 FEM 回放（基准、索力、根数），同一进程后续每循环2次；
触发局部索力候选验证的循环额外增加1次。连续 C 轮通常共 `2*C+1` 次，最坏再增加至多 C 次；
跨进程续算首轮需重新读取检查点方案。

终端使用原地刷新的固定仪表板，同时显示24个施工组的背索/中跨索根数、A激活/最终/目标
梁端 `z`、A激活/最终塔端 `x`、最终背索/中跨索应力，以及循环进度、耗时和 ETA，
不再逐工况追加打印。
...
历史分别记录索力轮和根数轮的候选、验收与指标。索力采用约30 MPa的默认等效应力步长，
