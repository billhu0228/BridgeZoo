...
def test_staged_matches_opensees_corot():
    """同一施工计划：自研(线性) vs OpenSees(corot)，小位移下 < 1%。"""
    ...
    pre = [wg * L * math.hypot(i * L, H) / H for i in range(1, n + 1)]
    plan = build_staged_cantilever(n_seg=n, seg_len=L, tower_height=H, wg=wg,
    ...
    for c in hd:
        assert abs(hd[c] - ho[c]) < 0.01 * scale
    # 末阶段悬臂端挠度
    tip = n
    da = rd.records[-1].disp[tip][1]
    db = ro.records[-1].disp[tip][1]
    assert abs(da - db) <= 0.01 * max(abs(da), abs(db), 1e-9)
