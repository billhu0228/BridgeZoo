...
改善即可接受，允许暂时扰动位移；后续索力轮必须在新根数和新刚度下重新寻找位移平衡位置。
...
诊断，不再否决减根或增根。验收后的根数和受扰动位移成为下一循环的基准，下一索力轮必须
在改变后的索刚度 `EA` 下重新寻找 A/B 位移平衡。拒绝根数候选不会回滚已验收的索力，也
不会再因为整轮拒绝而同步减半所有索组缓存。通常全新首循环为3次完整施工回放（基准、索力候选、根数候选），同一进程后续循环
复用上轮最终阶段记录而新增2次；仅在全量索力候选失败且存在可保留的局部改善分量时，
该循环额外增加1次局部候选回放。因此连续 C 轮通常为 `2*C+1` 次，最坏再增加至多 C 次。
...
估算方案写成最佳设计。v11 可迁移读取 v4/v5/v6/v7/v8/v9/v10 数值状态；缺失的根数记忆初始化
...
显示背/中索根数、A激活/最终/目标梁端 `z`、A激活/最终塔端 `x` 和最终应力。工程循环输出继续使用逐索
